...
assert_contains "only the valid entry is attempted" "$restore_log" "(1 attempted)"
...
assert_contains "failed pane is excluded from attempts" "$restore_log" "(1 attempted)"
export MOCK_FAIL_CLEAR_PANE=''
...
assert_contains "duplicate pane is excluded from attempts" "$restore_log" "(1 attempted)"
