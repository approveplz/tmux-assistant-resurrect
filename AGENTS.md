...
| **Codex keeps its thread writer lock and rollout JSONL open** | Primary exact PID-to-session mapping. The save script uses rollout `session_meta.source` to exclude open subagent threads, accepts one root UUID, and rejects ambiguity; this avoids guessing among sessions that share a cwd. | On Linux/WSL, inspect `/proc/<pid>/fd`; on macOS, run `lsof -p <pid>`. Check rollout first lines: the pane root has a non-subagent source, while spawned threads have `source.subagent` metadata. |
